...
def tokenize(string):
    ...
    """
    return [w for w in simpleTokenize(string) if not w in stopwords]
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda x:(x[0],tokenize(str(x[1]))))
# print amazonRecToToken[0]
googleRecToToken = googleSmall.map(lambda x:(x[0],tokenize(str(x[1]))))
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(20,lambda (k,v):-1*len(v))#.collect()
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (k,v):set(v))#.distinct()
    tokenCountPairTuple = uniqueTokens.map(lambda x:(x,1.0))
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a,b:a+b)
    return (tokenSumPairTuple.map(lambda (k,v):(k,N/v)))
...
tokenSmallestIdf = idfsSmall.takeOrdered(5, lambda s: s[1])[0]
...
def cossim(a, b):
    ...
    """
    return 1.0*dotprod(a,b)/(norm(a)*norm(b))
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (a,b,cs):(b+" "+a, cs))

trueDupsRDD = (sims
               .join(goldStandard))
# print trueDupsRDD.collect()
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda x:(x[0],tokenize(str(x[1]))))
googleFullRecToToken = google.map(lambda x:(x[0],tokenize(str(x[1]))))
...
idfsFullWeights = idfs(amazonFullRecToToken.union(googleFullRecToToken)).collectAsMap()
idfsFullBroadcast = sc.broadcast(idfsFullWeights)

# Pre-compute TF-IDF weights.  Build mappings from record ID weight vector.
amazonWeightsRDD = amazonFullRecToToken.map(lambda x:(x[0],tfidf(x[1],idfsFullWeights)))
# print amazonWeightsRDD.collect()
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda x:norm(x[1])).collect()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda x:norm(x[1])).collect()
...
def invert(record):
    ...
    ID=record[0]
    tokensList=record[1]
    for i in tokensList.keys():
        pairs.append((i,ID))
    return (pairs)

amazonInvPairsRDD = (amazonWeightsRDD
                    .flatMap(lambda x:invert((x[0],x[1])))
                    .cache())
# print amazonWeightsRDD.map(lambda x:invert((x[0],x[1]))).collect()
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(lambda x:swap(x)).groupByKey().mapValues(list)
                .cache())
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = computeSimilarity(((amazonRec,tokens),(googleRec,tokens)))
    value = s[2]
